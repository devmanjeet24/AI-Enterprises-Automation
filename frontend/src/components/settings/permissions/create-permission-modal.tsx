...
export function CreatePermissionModal({ open, onClose }: CreatePermissionModalProps) {
  ...
  const slugError = slug.trim() ? validatePermissionSlug(slug) : null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <button
        type="button"
        aria-label="Close create permission modal"
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h2
                id="create-permission-title"
                className="mt-0.5 text-lg font-medium tracking-[-0.02em] text-foreground"
              >
                Create permission
              </h2>
              <p className="mt-1 text-[13px] text-muted-foreground">
                Slugs must use resource:action format (for example, reports:read).
              </p>
            </div>
          </div>
          <Button
