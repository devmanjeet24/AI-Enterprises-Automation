...
export function VoiceAgentCard({ agent }: VoiceAgentCardProps) {
  ...
  return (
    <DashboardCard variant="default" accent="emerald" className="h-full p-5">
      <Link href={`/voice-ai/${agent.id}`} className="group block">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h3 className="truncate text-[15px] font-medium tracking-[-0.01em] text-foreground group-hover:text-brand">
                {agent.name}
              </h3>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <Link
          ...
          className="flex min-w-0 items-center gap-1.5 text-[12px] text-muted-foreground transition-colors hover:text-brand"
        >
          <User className="size-3.5 shrink-0" />
          <span className="truncate">View linked AI employee</span>
        </Link>
